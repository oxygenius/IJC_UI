...
/**
 * Deelt de groepen in op basis van aanwezigheid en methode. Mogelijke methoden
 * zijn 1. Zonder doorschuiven 2. 3 of 4 spelers schuiven een groep omhoog.
...
public class GroepenIndeler {
	...
	/**
	 * Maak de groepsindeling voordat de wedstrijden worden bepaald. Spelers die
	 * afwezig zijn, worden uit de speellijst verwijderd. Indien van toepassing,
	 * worden 3 of 4 spelers doorgescheven naar een hogere groep. De hogere
	 * groep eindigt altijd met een even aantal spelers. Bij oneven spelers
	 * worden er dus 3 doorgeschoven en bij even spelers mogen er 4
	 * doorschuiven.
	 *
	 * @param aanwezigheidsGroepen
	 *            Overzicht spelers per groen met aanwezigheidsinfo
	...
	public Groepen maakGroepsindeling(Groepen aanwezigheidsGroepen) {
		...
		// Er wordt een nieuwe groepen gemaakt, welke stapsgewijs gevuld gaat
		// worden.
	...
	/**
	 * Werk de groepsindeling van één groep bij voordat de wedstrijden worden
	 * bepaald. Spelers die afwezig zijn, worden uit de speellijst verwijderd.
	 * Indien van toepassing, worden 3 of 4 spelers doorgescheven naar een
	 * hogere groep. De hogere groep eindigt altijd met een even aantal spelers.
	 * Bij oneven spelers worden er dus 3 doorgeschoven en bij even spelers
	 * mogen er 4 doorschuiven.
	 * 
	 * @param aanwezigheidsGroepen
	 *            Overzicht spelers per groep met aanwezigheidsinfo
	 * @param wedstrijdGroepen
	 *            Huidige wedstrijdgroepen
	 * @param groepID
	 *            Specificeert de groep die ge-update moet worden
	 * @return de wedstrijdgroepen
	 */
	public Groepen maakGroepsindeling(Groepen aanwezigheidsGroepen, Groepen wedstrijdGroepen, int groepID) {
		logger.log(Level.INFO,
				"Maken groepsindeling voor groep" + aanwezigheidsGroepen.getGroepById(groepID).getNaam());
		...
		// Zoek spelers uit deze groep die doorgeschoven zijn naar een hogere
		// groep
		...
		// Creeer nieuwe groep
		// Neem alle aanwezige spelers hier in op, behalve degene die al
		// doorgeschoven
	...
	/**
	 * Schuif spelers door. Laatste speler wordt alleen doorgeschoven indien dit
	 * tot een even aantal spelers in de nieuwe groep leidt.
	 * 
	 * @param wedstrijdGroepen
	 * @param aanwezigheidsGroepen
	 */
	private void doorschuiven(Groepen wedstrijdGroepen, Groepen aanwezigheidsGroepen) {
		...
		// Doorloop hoogste groep tot Ã©Ã©n na laagste groep. In de laagste
		// groep
		// kunnen geen spelers inschuiven
		ArrayList<Groep> groepen = wedstrijdGroepen.getGroepen();
		for (int i = 0; i < groepen.size() - 1; ++i) {
			logger.log(Level.FINE,
					"Doorschuiven van groep " + groepen.get(i).getNaam() + " naar " + groepen.get(i).getNaam());
			...
			for (int j = 1; j <= aantal; ++j) {
				...
				if ((s != null) && s.isAanwezig()) {
					if ((j == aantal) && (aantal == 1)) {
						// Alleen doorschuiven als speler 1 niet meer ingehaald
						// kan worden
						...
						if (naarGroep.size() % 2 != 0) {
							logger.log(Level.FINE,
									"Speler doorgeschoven, laatste doorschuiver maar door om even aantal ");
							naarGroep.add(new Speler(s));
							vanGroep.remove(s);
						}
					} else {
	...
	/**
	 * Op basis van periode en ronde gegevens wordt bepaald of er wel of niet
	 * wordt doorgeschoven
	 *
	 * @param periode
	 *            Huidige periode
	 ...
	 *            Ronde in de periode
	 * @param serie
	 *            serie wedstrijden binnen de ronde
	 * @return
	 */
	...
	/**
	 * Bepaal het het aantal series dat tijdens een ronde wordt gespeeld. Dit is
	 * afhankelijk van de groep, de periode en de ronde.
	 *
	 * @param groep
	 *            Niveau van de groep
	 * @param periode
	...
	/**
	 * Update wedstrijden voor één groep. Wedstrijden voor alle andere groepen
	 * blijven ongewijzigd.
	 * 
	 * @param wedstrijden
	 *            Huidige wedstrijden voor alle groepen
	 * @param wedstrijdgroepen
	 *            Huidige wedstrijdgroepen voor all groepen
	 * @param groepID
	 *            ID van groep om opnieuw te bepalen
	 * @return update van wedstrijden met nieuwe wedstrijden voor specifieke
	 *         groep
	 */
	...
	private Groepswedstrijden maakWedstrijdenVoorGroep(int periode, int ronde, Groep wedstrijdgroep) {
		...
		for (int i = 0; i < speelrondes; i++) {
			...
			if (i > 0) {
				if (!(trioloc == 0)) {
					...
					gws.setFuzzyMatrix(matrix2);
				} else {
					gws.setFuzzyMatrix(matrix);
				}
			} else {
				gws.setFuzzyMatrix(matrix);
			}
			int[][] fmatrix = gws.getFuzzyMatrix();
			...
			if (groep.getNaam().equals("Pionnengroep")) {
				groep.SpelersNamenopvolgorde(tri,indexrow);
		...
		// logger.log(Level.INFO, "Maken van een trio vanwege oneven aantal
		// spelers");
		...
		// logger.log(Level.INFO, "Spelers in trio " + sid1.getInitialen() + " "
		// + sid2.getInitialen() + " " + sid3.getInitialen());
		...
		// }
		// for (int i = 0; i < speelrondes; ++i) {
		// int minverschil = bepaalMinimaalVerschil(groep, periode, ronde, i +
		// 1);
		...
		// serie = maakSerie(groep, gepland, aantalSpelers, minverschil,
		// ignoreTgns, ronde);
		// ignoreTgns++;
		// }
		//
	...
	public Serie maakSerie(Groep groep, boolean[] gepland, int aantalSpelers, int minverschil, int ignoreTgn,
			int ronde) {
		Serie serie = new Serie();
		int mv = minverschil;
		while (mv >= 0) {
			Serie s = planSerie(serie, groep.getSpelers(), gepland, aantalSpelers, minverschil, ignoreTgn,
					groep.getNiveau(), 1, ronde);
			if (s != null) {
				return s;
			}
			mv--;
	...
	private Serie planSerie(Serie serie, ArrayList<Speler> spelers, boolean[] gepland, int teplannen, int minverschil,
			int ignoreTgn, int niveau, int diepte, int ronde) {
		...
		if ((doorgeschovenID >= 0) && (ronde == 1)) {
			...
			while (zoekId != -1) {
				...
				if (!s1.isGespeeldTegen(s2, minverschil) && (s2.getGroep() != s1.getGroep())) {
					...
					Serie s = planSerie(serie, spelers, gepland, teplannen - 2, minverschil, ignoreTgn, niveau,
							diepte + 1, ronde);
					if (s != null) {
	...
	/**
	 * Vind de laatste ongeplande speler beginnende bij start en terugzoekende
	 * naar 0
	 * 
	 * @param gepland
	 * @param start
		...
		for (i = 1; i <= wedstrijdgroep.getAantalSpelers(); i++) {
			...
			for (j = 1; j <= wedstrijdgroep.getAantalSpelers(); j++) {
				...
				if (i == j) {
					...
					if (serie == 0) {
						tegenstanders = wedstrijdgroep.getSpelerByID(i)
								.getGespeeldTegen(wedstrijdgroep.getSpelerByID(j));
					} else {
						tegenstanders = wedstrijdgroep.getSpelerByID(i)
								.getGespeeldTegen(wedstrijdgroep.getSpelerByID(j));
					}
	...
	public Groep updateSpelers(Groep groep, Serie serie) {
		...
		for (Speler speler : groep.getSpelers()) {
			...
			if (wedstrijd != null) {
				...
				System.out.println("Hmmm, geen wedstrijd voor speler niet gevonden.... Zit misschien in een trio.\n");
			}
		}
		return groep;
	}
	 ...
	 * ArrayList<Integer> trio = new ArrayList<>(); if
	 * (groep.getSpelers().size() == 3) { // 3 spelers, dus maak gelijk trio
	 * trio.add(groep.getSpelers().get(0).getId());
	 * trio.add(groep.getSpelers().get(1).getId());
	 * trio.add(groep.getSpelers().get(2).getId()); return trio; } int spelerID
	 * = groep.getSpelers().size() / 2; int minDelta = 1; int plusDelta = 1; int
	 * ignore = 0; boolean doorzoeken = true; while (doorzoeken) { Speler s1 =
	 * groep.getSpelerByID(spelerID); Speler s2 = groep.getSpelerByID(spelerID -
	 * minDelta); Speler s3 = groep.getSpelerByID(spelerID + plusDelta); if
	 * (isGoedTrio(s1, s2, s3, ignore)) { trio.add(s1.getId());
	 * trio.add(s2.getId()); trio.add(s3.getId()); return trio; } else { if ((s2
	 * == null) || (s3 == null)) { if (ignore > 4) { doorzoeken = false; }
	 * ignore += 1; minDelta = 1; plusDelta = 1; } else { if (minDelta >
	 * plusDelta) { plusDelta++; } else { minDelta++; } } } } return trio; }
	 */
	/**
	 * Stel vast op het meegegeven trio een goed trio is conform de regels.
	 * 
	 * @param s1
	 *            Speler 1
	 * @param s2
	 ...
	 *            Speler 3
	 * @param ignore
	 *            Aantal te negeren rondes in het verleden
	 * @return
	 */
	/**
	 * private boolean isGoedTrio(Speler s1, Speler s2, Speler s3, int ignore) {
	 * if ((s1 != null) && (s2 != null) && (s3 != null)) { return
	 * !s1.isGespeeldTegen(s2, ignore) && !s1.isGespeeldTegen(s3, ignore) &&
	 * !s2.isGespeeldTegen(s1, ignore) && !s2.isGespeeldTegen(s3, ignore) &&
	 * !s3.isGespeeldTegen(s1, ignore) && !s3.isGespeeldTegen(s2, ignore); }
	 * else { return false; } }
	 */
}
